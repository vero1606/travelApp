            ...
            {listingSubmitted ? (
              <div style={{ padding: '2.5rem', textAlign: 'center' }}>
                ...
                <p style={{ color: '#666', fontSize: '0.92rem', marginBottom: '1.5rem' }}>
                  <strong>{listingForm.title}</strong> in {listingForm.location} has been successfully listed.
                </p>
                <div style={{ background: '#f8faff', borderRadius: 12, padding: '1rem', textAlign: 'left', marginBottom: '1.5rem' }}>
                  {[
                    { label: 'Title', value: listingForm.title },
                    { label: 'Location', value: listingForm.location },
                    { label: 'ype', value: listingForm.type },
                    { label: 'Price', value: listingForm.price },
                    { label: 'Bedrooms', value: listingForm.bedrooms },
                    { label: 'Max Guests', value: listingForm.guests },
                  ].map(row => (
              ...
              <div style={{ padding: '1.5rem' }}>
                {[
                  ...
                  <div key={f.key} style={{ marginBottom: '1rem' }}>
                    ...
                    <input type={f.type} placeholder={f.placeholder} value={listingForm[f.key]}
                ...
                <div style={{ marginBottom: '1rem' }}>
                  ...
                  <textarea placeholder="Describe your property..." value={listingForm.description}
                ...
                <div style={{ marginBottom: '1.3rem' }}>
                  ...
                  {listingForm.imagePreview && (
                    <img src={listingForm.imagePreview} alt="preview" style={{ width: '100%', height: 160, objectFit: 'cover', borderRadius: 10, marginBottom: 8, display: 'block' }} />
                  )}
                  <input type="file" accept="image/*"
                    onChange={e => {
                      const file = e.target.files[0];
                      if (file) setListingForm({ ...listingForm, image: file, imagePreview: URL.createObjectURL(file) });
                    }}
                    style={{ width: '100%', fontSize: '0.88rem', color: '#555', padding: '0.4rem 0' }}
                  />
                </div>
                ...
                <button
                  onClick={() => {
                    if (listingForm.title && listingForm.location && listingForm.price) {
                      setListings([...listings, { ...listingForm, id: Date.now() }]);
                      setListingSubmitted(true);
                    }
                  }}
                  style={{ width: '100%', padding: '0.85rem', background: 'linear-gradient(135deg, #1e3a5f, #3b5bdb)', color: '#fff', border: 'none', borderRadius: 12, fontSize: '1rem', fontWeight: 700, cursor: 'pointer' }}>
                  Submit Listing
                </button>
              </div>
            )}
        ...
        {activeTab === 'list property' && (
          <div>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                  {listings.map(l => (
                    <div key={l.id} style={{ background: '#fff', borderRadius: 14, boxShadow: '0 2px 16px rgba(0,0,0,0.07)', overflow: 'hidden' }}>
                      {l.imagePreview && (
                        <img src={l.imagePreview} alt={l.title} style={{ width: '100%', height: 180, objectFit: 'cover', display: 'block' }} />
                      )}
                      ...
                      {editingId === l.id ? (
                        ...
                        <div style={{ padding: '1.2rem' }}>
                          {[
                            ...
                            <div key={f.key} style={{ marginBottom: '0.8rem' }}>
                              <label style={{ display: 'block', fontSize: '0.75rem', fontWeight: 600, color: '#444', marginBottom: 4, textTransform: 'uppercase', letterSpacing: '0.04em' }}>{f.label}</label>
                          ...
                          <div style={{ marginBottom: '1rem' }}>
                            <label style={{ display: 'block', fontSize: '0.75rem', fontWeight: 600, color: '#444', marginBottom: 4, textTransform: 'uppercase', letterSpacing: '0.04em' }}>Description</label>
                          ...
                          <div style={{ display: 'flex', gap: '0.7rem' }}>
                            ...
                            <button onClick={() => { setListings(listings.map(x => x.id === l.id ? { ...x, ...editForm } : x)); setEditingId(null); }}
                              style={{ flex: 1, padding: '0.6rem', borderRadius: 8, border: 'none', background: 'linear-gradient(135deg, #1e3a5f, #3b5bdb)', color: '#fff', cursor: 'pointer', fontWeight: 700, fontSize: '0.88rem' }}>
                              Save
                            </button>
                          </div>
                        </div>
                        ...
                        <div style={{ padding: '1.2rem', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                          ...
                          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem', alignItems: 'flex-end', flexShrink: 0, marginLeft: '1rem' }}>
                            ...
                            <button
                              onClick={() => { setEditingId(l.id); setEditForm({ title: l.title, location: l.location, price: l.price, description: l.description }); }}
                              style={{ background: '#f0f4ff', color: '#3b5bdb', border: 'none', padding: '4px 12px', borderRadius: 20, fontSize: '0.78rem', fontWeight: 600, cursor: 'pointer' }}>
                                Edit
                            </button>
                            <button onClick={() => setListings(listings.filter(x => x.id !== l.id))}
                              style={{ background: '#fee2e2', color: '#dc2626', border: 'none', padding: '4px 12px', borderRadius: 20, fontSize: '0.78rem', fontWeight: 600, cursor: 'pointer' }}>
                                Delete
                            </button>
                          </div>
                        </div>
            ...
            <div style={{ textAlign: 'center' }}>
              <button onClick={() => setShowListProperty(true)} style={{
